...
namespace VTKRender {
	 ...
	 */
	VTKRender::DicomDirTreeModel::DicomDirTreeModel(const QStringList& headers, const QString& data, QObject* parent)
	 ...
	 * patient  这棵树要存储的病人实例
	 */
	VTKRender::DicomDirTreeModel::DicomDirTreeModel(const QStringList& headers, const Patient& patient, QObject* parent)
	{
		QVector<QVariant> rootData;
		foreach(QString header, headers)
			rootData << header;
	 ...
	 * 析构函数
	 */
	VTKRender::DicomDirTreeModel::~DicomDirTreeModel()
	{
		delete rootItem;
	}
	/*
	 *  返回列数
	 */
	int VTKRender::DicomDirTreeModel::columnCount(const QModelIndex& /* parent */) const
	{
	 ...
	 */
	QVariant VTKRender::DicomDirTreeModel::data(const QModelIndex& index, int role) const
	{
		if (!index.isValid())
			return QVariant();
	 ...
	 *
	 */
	Qt::ItemFlags VTKRender::DicomDirTreeModel::flags(const QModelIndex& index) const
	{
		if (!index.isValid())
			return 0;
	 ...
	 * 用QModelIndex索引Item
	 */
	DicomTreeItem* VTKRender::DicomDirTreeModel::getItem(const QModelIndex& index) const
	{
		if (index.isValid()) {
	 ...
	 */
	int VTKRender::DicomDirTreeModel::getLevel(const QModelIndex& index) const
	{
	 ...
	 *
	 */
	QVariant VTKRender::DicomDirTreeModel::headerData(int section, Qt::Orientation orientation, int role) const
	{
	 ...
	 *
	 */
	QModelIndex VTKRender::DicomDirTreeModel::index(int row, int column, const QModelIndex& parent) const
	{
	 ...
	 * 插入列
	 */
	bool VTKRender::DicomDirTreeModel::insertColumns(int position, int columns, const QModelIndex& parent)
	{
		bool success;
	 ...
	 * 插入行
	 */
	bool VTKRender::DicomDirTreeModel::insertRows(int position, int rows, const QModelIndex& parent)
	{
		DicomTreeItem* parentItem = getItem(parent);
		bool success;
	 ...
	 */
	QModelIndex VTKRender::DicomDirTreeModel::parent(const QModelIndex& index) const
	{
		if (!index.isValid())
			return QModelIndex();
	 ...
	 * 删除列
	 */
	bool VTKRender::DicomDirTreeModel::removeColumns(int position, int columns, const QModelIndex& parent)
	{
		bool success;
	 ...
	 * 删除行
	 */
	bool VTKRender::DicomDirTreeModel::removeRows(int position, int rows, const QModelIndex& parent)
	{
		DicomTreeItem* parentItem = getItem(parent);
		bool success = true;
	 ...
	 * 行数
	 */
	int VTKRender::DicomDirTreeModel::rowCount(const QModelIndex& parent) const
	{
		DicomTreeItem* parentItem = getItem(parent);
	 ...
	 */
	bool VTKRender::DicomDirTreeModel::setData(const QModelIndex& index, const QVariant& value, int role)
	{
		if (role != Qt::EditRole)
			return false;
	 ...
	 * 改表头
	 */
	bool VTKRender::DicomDirTreeModel::setHeaderData(int section, Qt::Orientation orientation, const QVariant& value, int role)
	{
	 ...
	 * 使用缺省数据文件构造树
	 */
	void VTKRender::DicomDirTreeModel::setupModelData(const QStringList& lines, DicomTreeItem* parent)
	{
	 ...
	 * 使用patient对象构造树
	 */
	void VTKRender::DicomDirTreeModel::setupModelData(const Patient& patient, DicomTreeItem* parent)
	{
